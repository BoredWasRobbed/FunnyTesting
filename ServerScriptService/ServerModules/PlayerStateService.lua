...
function PlayerStateService:ApplyMovement(subject)
	...
	if machine:HasState("Dead") or machine:HasState("Frozen") or machine:HasState("Ragdolled") or machine:HasState("Stunned") or machine:HasState("GuardBroken") then
		...
		jumpHeight = 0
	elseif machine:HasState("Attacking") then
		local attackData = machine:GetStateData("Attacking") or {}
		walkSpeed = tonumber(attackData.WalkSpeed) or self.Movement.BusyWalkSpeed
	elseif machine:HasState("UsingSkill") or machine:HasState("MoveParry") then
